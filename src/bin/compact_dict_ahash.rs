...
fn main() {
    ...
    let mut dic: Dict<i32, dict::ahash::AHashStrHash> = Dict::new(N);
    // Insert
    ...
    let (program_name, file_name) = ("compact_dict_ahash.rs", "compact-dict-ahash-rs.json");
    ...
    let csv_path = Path::new("results").join("compact-dict-ahash-rs.csv");
